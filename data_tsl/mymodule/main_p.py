...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # cd C:\my_games\tsl
        # git init
        # git remote add origin https://github.com/아이디/tsl.git
        ...
        # pyinstaller --noconfirm --name tsl --icon "C:\my_games\tsl\tsl_macro.ico" --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data "C:\my_games\tsl\data_tsl;data_tsl" --add-data "C:\my_games\tsl\mysettings;mysettings" --add-data "C:\my_games\tsl\tsl_macro.ico;." --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" .\main.py
        # pyinstaller --noconfirm --name tsl --icon "C:\my_games\icon\tsl_macro.ico" --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data "C:\my_games\tsl\data_tsl;data_tsl" --add-data "C:\my_games\tsl\mysettings;mysettings" --add-data "C:\my_games\icon\tsl_macro.ico;." --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" .\main.py
        # 업데이트버젼
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        # 마을 의뢰
        self.com_group6 = QGroupBox('기본스케쥴')
        cb6 = QComboBox()
        list6 = ['스케쥴 선택', '각종템받기', '거래소등록', '자동사냥']
        cb6.addItems(list6)
        vbox6 = QHBoxLayout()
        vbox6.addWidget(cb6)
        maul_add = QPushButton('행동 추가')
        maul_add.clicked.connect(self.onActivated_maul_add)
        ...
        # 던전 종류
        self.dun_group_2 = QGroupBox('던전')
        dun_g2_name = QComboBox()
        # list4 = ['던전 선택', '일반_업보', '일반_지옥', '일반_죄악', '일반_저주', '특수_마족', '특수_아르카스', '파티_묘지']
        # dun_g2_list = ['던전 선택', '다크디멘젼', '레이드', '기간토마키아']
        dun_g2_list = ['던전 선택', '특수_버려진기지', '특수_귀사당', '특수_별의낙원', '파티_버려진광산', '파티_달빛사원', '파티_제르비나의석상', '이벤트_이벤트']
        dun_g2_name.addItems(dun_g2_list)

        dun_g2_stair = QComboBox()
        dun_g2_stair_list = ['층', '1', '2', '3', '4']
        ...
        #던전
        dun_g1_name.activated[str].connect(self.onActivated_dunjeon_1)  # 퀘스트 종류
    ...
    def onActivated_dunjeon_1_add(self):
        char_ = onCharacter
        dun_ = "퀘스트_" + str(onDunjeon_1) + "_" + str(onDunjeon_1_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_1 == '퀘스트 선택' or onDunjeon_1 == 'none' or onDunjeon_1_level == 0 or onDunjeon_1_level == "챕터":
    ...
    def onActivated_dunjeon_2_add(self):
        char_ = onCharacter
        dun_ = "던전_" + str(onDunjeon_2) + "_" + str(onDunjeon_2_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
    ...
    def onActivated_dunjeon_3_add(self):
        char_ = onCharacter
        dun_ = "미션_" + str(onDunjeon_3_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_3 == '던전 선택' or onDunjeon_3_level == 0 or onDunjeon_3_level == "구역":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0 or onDunjeon_3_level == 0 or onDunjeon_3_level == "구역":
            print('char_', char_)
            print('dun_', dun_)
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        if game_title == True:
                            ...
                            if look_title == True:
                                ...
                                elif "자동사냥" in result_schedule_:
                                    jadong_start(v_.now_cla)

                                elif "각종템받기" in result_schedule_:
